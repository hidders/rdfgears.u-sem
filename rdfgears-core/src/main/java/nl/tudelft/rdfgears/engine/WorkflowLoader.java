...
public class WorkflowLoader {
	...
	public static Workflow loadWorkflow(String workflowId) throws WorkflowLoadingException{
		...
		if (workflowId==null || workflowId.equals("")){
			throw new WorkflowLoadingException("No workflow was specified to load.");
		}
...
//			throw ex;
	...
	public void load() throws CircularWorkflowException, WorkflowLoadingException {
		...
		/**
		 * it is possible that there is an input specified in the XML, but it is not connected to 
		 * the workflow output via processors. Add it anyway, as some typing properties require
		 * an input to be available 
		 */
		...
		for (int i=0; i<nodeList.getLength(); i++){
			...
			if (item instanceof Element){
				...
				if (! workflowInputName.equals(""))
					buildingWorkflow.requireInput(workflowInputName); // was probably already added
			}
		}
	...
	private Element getWorkflowInputListElement(){
		Element workflowElem = XMLUtil.getFirstSubElementByName(xmlDoc.getDocumentElement(), "workflow");
		return XMLUtil.getFirstSubElementByName(workflowElem , "workflowInputList");
	}
...
//				System.exit(0);
	...
	public static Class<?> loadFunctionClass(String funcName){
		...
		if(theClass==null){
			...
			try { // WorkflowĹoaders's classloader can access the jars in a jetty webapp. 
