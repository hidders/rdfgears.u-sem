...
public class BagGroup extends AtomicRGLFunction {
	 ...
	 * 
	 */
	class GroupedBag extends BagValue {
		private RecordValue[] records;
		...
		public GroupedBag(BagValue bag) {
			...
			if (true){ // if we cannot reuse parent array  
				...
				records = new RecordValue[recordList.size()]; 
		...
		public int size() {
			if (size < 0){
				...
				size = 0;
				RecordValue comparableRecord = records[finger]; // we will make a bag of values that all have the same groupable-variables as this
				finger++; // we already check that one :-)
				while(finger < toIndex){
					RecordValue rv = records[finger++];
		...
		class GroupedBagIterator implements Iterator<RGLValue> {
			...
			public RecordValue next() {
				...
				RecordValue comparableRecord = records[groupFromIndex]; // make a bag of values that all have the same groupable-variables as this
				...
				while(groupToIndex < toIndex){
					RecordValue rv = records[groupToIndex];
					if (comp.compare(comparableRecord, rv)!=0) //  rv is in different group
						break;
					
					groupToIndex++;
				}
				ModifiableRecord modRec = ValueFactory.createModifiableRecordValue(fiMapWithGroupField);
 ...
 *
 */
interface RecordComparator extends Comparator<RecordValue> {
}
...
class RecordComparatorVarField implements RecordComparator {
	...
	@Override
	public int compare(RecordValue o1, RecordValue o2) {
