...
public class RemoteSelectBehavior extends AbstractSelectBehavior {
	...
	public RGLValue simpleExecute(ValueRow inputRow) {
		...
		BagValue queryingBagValue = new BatchedRemoteQueryingBagValue(sparqlFunc.getQuery(), inputRow, sparqlFunc.getEndpointURI()); // create remote query
		...
		if (Engine.getConfig().do_greedyLoadingOfRemoteQueries()){
			...
			// disable lazy loading, load EVERYTHING, to find SPARQL endpoint errors ASAP
			Iterator<RGLValue> iterator = queryingBagValue.iterator();
			while (iterator.hasNext()) 
				iterator.next();
			
			Engine.getLogger().debug("remote query gave "+queryingBagValue.size()+ " results");
		}
 ...
 * 
 * Takes a query, an inputRow with binding variables (currently not used, not implemented) and an
 * endpointURI to execute the query on. 
 * 
 * The resulting bag lazily queries the endpoint in batches of QUERY_BATCH_SIZE results. This is necessary
 * because some endpoints implicitly LIMIT 2000.   
 * 
 * @author Eric Feliksik
 *
 */
class BatchedRemoteQueryingBagValue extends StreamingBagValue {
	
	public final static int QUERY_BATCH_SIZE = Engine.getConfig().getRemoteSparqlSelectBatchSize(); /* the size of each batch */
	...
	/**
	 * provide a non-null endpointURI to create lazybag for endpoint querying 
	 */
	public BatchedRemoteQueryingBagValue(Query query, ValueRow inputRow, String endpointURI) {
	...
	class RemoteQueryIter implements Iterator<RGLValue> {
		private int nextElemNr = 0; /* the next element the iterator points to */
		
		/* a buffer of remotely fetched values */
		private ArrayList<RGLValue> buffer = new ArrayList<RGLValue>(QUERY_BATCH_SIZE>0 ? QUERY_BATCH_SIZE : 16 );
		
		/* index of the next element in the buffer, related to nextElemNr (see invariant) */
		private int nextElemBufferIx = 0; 
		
		private int buffersIterated = 0; // the number of buffers we already consumed 
		...
		public boolean hasNext() {
			...
			if (nextElemBufferIx >= buffer.size() ){
				return false; // The buffer is filled, but doesn't contain that many elements;
			}
			return true;
		}
		 ...
		 ************************************************************************************
		 * 
		 *   Batched remote query mechanism ; only works if isEndpointQuery()==true, so this 
		 *   may be moved to a separate class
		 * 
		 ************************************************************************************
		 ************************************************************************************/
		...
		/**
		 * @return the limit for the next query, assuming the next query should be batched 
		 */
		private long getNextLimit(){
			if (originalQuery.hasLimit() && fetchedResults + QUERY_BATCH_SIZE > originalQuery.getLimit()){
		...
		/**
		 * Fetch results per QUERY_BATCH_SIZE. This is not only lazy, but more importantly, it's necessary because Virtuoso
		 * endpoints often give a max of  QUERY_BATCH_SIZE results. So we request repetitively a LIMIT of  QUERY_BATCH_SIZE, 
		...
		private void fillBuffer(List<RGLValue> buffer) {
			...
			/* We pass null as inputRow because unfortunately, initial bindings are not supported for 
			 * remote queries in our ARQ version. 
			 * Maybe later, when we can rewrite the query and bind it ourselves. */
			QueryExecution qexec = QueryUtil.createRemoteQueryExecution(executeQuery, inputRow, endpointURI);
			...
			while (true) {
				try {
					Engine.getLogger().info("Executing remote query on "+endpointURI+"\n"  +executeQuery);
					resultsInThisBatch = (int) QueryUtil.executeSelectQueryToRecordList(qexec, queryFieldMap, buffer);
					if (doQueryBatching && buffer.size()>QUERY_BATCH_SIZE){
						Engine.getLogger().warn("Buffer size is "+buffer.size()+", but was expected to be <="+QUERY_BATCH_SIZE +". So your SPARQL endpoint is giving more values than requested!");
					} 
					break; // done! 
				}	
