...
//	private FieldMappedValueRow previousRow; // recycling disabled
	...
	public FunctionProcessor(RGLFunction function, String id){
		...
		if (Engine.getConfig().lazinessDisabled()){
			Engine.getLogger().debug("Disabling laziness for function "+function.getFullName());
			function.isLazy = false;
		}
...
//						inputRow.setRecyclable(true);
				 ...
				 * The LazyRGLValueMapping is a lazy value that may return NULL upon evaluate(), 
				 * if a marked input port contains NULL */
				cachedResultValue = new LazyRGLValueMapping(this, inputRow);
				
				if (! function.isLazy()){ 
					/* laziness is disabled. (Actually the isLazy() doesn't apply to this function but to 
					 * the mapping around it... but that's unfortunately not configurable) */
					cachedResultValue = cachedResultValue.asBag(); 
				} 
			}
			...
			if (Engine.getConfig().pipeliningDisabled() && cachedResultValue.isBag()){
				cachedResultValue.prepareForMultipleReadings();
				BagValue bag = cachedResultValue.asBag();
				BagValue.getNaiveSize(bag); // iterate all the way and materialize it
			}
