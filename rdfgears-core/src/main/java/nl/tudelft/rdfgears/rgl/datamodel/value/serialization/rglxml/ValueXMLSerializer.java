...
/**
 * A visitor that serializes an RGL Value as RGL/XML, writing it to an Output Stream.
 * Included RDF graph values are embedded as RDF/XML within &lt;graph&gt; tags. 
 * 
 * 
...
public class ValueXMLSerializer extends ValueSerializer {
	...
	public void serialize(RGLValue value)  {
		...
		try {
			...
			xmlwriter.writeNamespace(RGLXML.nameSpacePrefix, RGLXML.nameSpaceFull ); 
	...
	public void visit(BooleanValue bool) {
		try {
			...
			xmlwriter.writeAttribute(RGLXML.attrBooleanValue, bool.isTrue() ? RGLXML.literalTrueValue : RGLXML.literalFalseValue); 
	...
	public void visit(LiteralValue literal) {
		try {
			...
			if (literal.getLiteralType()!=null){
				xmlwriter.writeAttribute(RGLXML.attrLiteralDatatype, literal.getLiteralType().getURI());
			} else if (literal.getLanguageTag()!=null){
				xmlwriter.writeAttribute(RGLXML.attrLiteralLang, literal.getLanguageTag());
			}
	...
	public void visit(RGLNull rglNull) {
		try {
			...
			if (rglNull.getErrorMessage()!=null)
				xmlwriter.writeAttribute(RGLXML.attrNullMessage, rglNull.getErrorMessage());
